...
void AnyDSL_Transpiler::transpile_function(const IDefinition *definition) {
  ...
  std::vector<std::string> argument_types =
      std::vector<std::string>(parameter_count);
  std::vector<std::string> argument_names =
      std::vector<std::string>(parameter_count);
  std::vector<std::string> argument_types_mangling =
      std::vector<std::string>(parameter_count);
  ...
  for (int k = 0; k < parameter_count; ++k) {
    ...
    argument_types_mangling[k] =
        type_to_string_for_mangling(parameter_type, true);
    argument_types[k] =
        is_stateless ? type_to_string_closure(parameter_type)
                     : type_to_string_for_mangling(parameter_type, false);
...
const std::string
AnyDSL_Transpiler::type_to_string_for_mangling(const IType *type,
                                               bool no_special_characters) {
  std::string ts;
  switch (type->get_kind()) {
  case IType::TK_ALIAS: {
    const IType_alias *t = as<IType_alias>(type);
    ts = type_to_string_for_mangling(t->get_aliased_type(),
  ...
  case IType::TK_VECTOR: {
    ...
    ts = type_to_string_for_mangling(t->get_element_type(),
                                     no_special_characters) +
  ...
  case IType::TK_MATRIX: {
    ...
    ts = type_to_string_for_mangling(v->get_element_type(),
                                     no_special_characters) +
  ...
  case IType::TK_ARRAY: {
    ...
    str += (type_to_string_for_mangling(t->get_element_type(),
                                        no_special_characters));
    if (no_special_characters) {
      str += "_arr";
    } else {
  ...
  case IType::TK_FUNCTION: {
    ...
    if (!no_special_characters) {
      ...
      str += ") -> ";
      str += type_to_string_for_mangling(t->get_return_type(),
                                         no_special_characters);
      ts = str;
      break;
    } else {
...
void AnyDSL_Transpiler::transpile_type_alias(const IDeclaration *decl) {
  ...
  add_to_code(" = ");
  add_to_code(
      type_to_string_for_mangling(ta->get_type_name()->get_type(), false));
  add_to_code(";\n\n");
}
...
void AnyDSL_Transpiler::transpile_struct(const IDeclaration *decl) {
  ...
  for (int i = 0; i < s->get_field_count(); i++) {
    ...
    add_to_code(": ");
    add_to_code(type_to_string_for_mangling(
        s->get_field_type_name(i)->get_type(), false));
    add_to_code(",\n");
  }
  indent_level--;
  add_to_code("}\n\n", true);
  ...
  for (int i = 0; i < s->get_field_count(); i++) {
    add_to_code("_");
    add_to_code(type_to_string_for_mangling(
        s->get_field_type_name(i)->get_type(), true));
  }
...
void AnyDSL_Transpiler::transpile_variable_declaration(
  ...
  for (int i = 0; i < v->get_variable_count(); i++) {
    ...
    add_to_code(
        type_to_string_for_mangling(v->get_type_name()->get_type(), false));
...
void AnyDSL_Transpiler::transpile_constant_declaration(
    const IDeclaration *decl) {
  ...
  for (int i = 0; i < c->get_constant_count(); i++) {
    ...
    add_to_code(": ");
    add_to_code(
        type_to_string_for_mangling(c->get_type_name()->get_type(), false));
...
void AnyDSL_Transpiler::transpile_expression_literal(
    const IExpression *pExpression, bool closure) {
...
void AnyDSL_Transpiler::transpile_expression_let(const IExpression *pExpression,
  ...
  for (int i = 0; i < l->get_declaration_count(); i++) {
    ...
    if (decl->get_kind() == IDeclaration::Kind::DK_VARIABLE) {
      ...
      std::string var_type = is_stateless
                                 ? type_to_string_for_mangling(t, false)
...
void AnyDSL_Transpiler::transpile_expression_unary(
  ...
  add_to_code("__");
  add_to_code(
      type_to_string_for_mangling(un->get_argument()->get_type(), true));
  add_to_code("_mdl_math(");
...
void AnyDSL_Transpiler::transpile_expression_binary(
  ...
  if (is_shorthand_assign_operator(bin->get_operator())) {
    ...
    add_to_code(" = ");
    add_to_code(
        binary_operator_to_string(get_shorthand_operator(bin->get_operator())));
    add_to_code("__");
    add_to_code(type_to_string_for_mangling(
        bin->get_left_argument()->get_type(), true));
    add_to_code("_");
    add_to_code(type_to_string_for_mangling(
        bin->get_right_argument()->get_type(), true));
    add_to_code("_mdl_math(\n");
    indent_level++;
    add_to_code("", true);
    ...
    case IExpression_binary::OK_ARRAY_INDEX:
      if (bin->get_left_argument()->get_type()->skip_type_alias()->get_kind() ==
          ...
          bin->get_left_argument()->get_type()->skip_type_alias()->get_kind() ==
              IType::Kind::TK_MATRIX) {
        add_to_code(type_to_string_for_mangling(
                        bin->get_left_argument()->get_type(), true) +
                    "_get(");
        dispatch_transpile_expression(bin->get_left_argument(), closure);
    ...
    case IExpression_binary::OK_ASSIGN: {
      ...
      if (bin->get_left_argument()->get_type()->skip_type_alias()->get_kind() ==
              IType::Kind::TK_VECTOR &&
          bin->get_right_argument()
                  ->get_type()
                  ->skip_type_alias()
                  ->get_kind() != IType::Kind::TK_VECTOR) {
        add_to_code(type_to_string_for_mangling(
            bin->get_left_argument()->get_type(), true));
        add_to_code("__");
        add_to_code(type_to_string_for_mangling(
...
void AnyDSL_Transpiler::transpile_expression_call(
  ...
  const IDefinition *callee_def =
      callee->get_definition() == nullptr
          ? callee->get_name()->get_qualified_name()->get_definition()
...
bool AnyDSL_Transpiler::is_assign_operator(
...
bool AnyDSL_Transpiler::is_prefix_operator(
    const IExpression_unary::Operator op) {
  switch (op) {
  case IExpression_unary::OK_BITWISE_COMPLEMENT:
  case IExpression_unary::OK_LOGICAL_NOT:
...
bool AnyDSL_Transpiler::is_shorthand_assign_operator(
...
IExpression_binary::Operator
AnyDSL_Transpiler::get_shorthand_operator(IExpression_binary::Operator op) {
  switch (op) {
  case IExpression_binary::OK_MULTIPLY_ASSIGN:
